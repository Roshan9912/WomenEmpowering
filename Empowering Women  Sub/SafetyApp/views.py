...
def PanicAction(request):
    if request.method == 'POST':
        ...
        with smtplib.SMTP_SSL('smtp.gmail.com', 465) as connection:
            email_address = 'kokkularamana225@gmail.com'
            email_password = 'vevp fzxx blre gahv'
            connection.login(email_address, email_password)
            connection.sendmail(from_addr="kokkularamana225@gmail.com", to_addrs=em, msg=username+" sent emergency message from location "+areaname+" to you.")
...
def UserLoginAction(request):
    ...
    if request.method == 'POST':
        ...
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'safety',charset='utf8')
        with con:
            cur = con.cursor()
        ...
        with con:
            ...
            for row in rows:
                if row[0] == username:
                    ...
                    break                
        if output == "none":
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'safety',charset='utf8')
